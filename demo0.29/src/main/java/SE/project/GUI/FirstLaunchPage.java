...
public class FirstLaunchPage extends JFrame {
	...
	// textField, textField_1, textField_2 for the dealership name, location, and
	// inventory capacity
	...
	public FirstLaunchPage() {
		...
		btnNewButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				try {
					new Dealership(textField.getText(), textField_2.getText(), Integer.valueOf(textField_1.getText()));
					// Navigate to login page
					SwingUtilities.invokeLater(() -> {
						dispose(); // Ensure the main frame is disposed
						new LoginPage();
						// loginPage.setVisible(true);
						// commented out because it is already set to visible in loginpage constructor
					});
