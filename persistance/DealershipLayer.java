...
public class DealershipLayer {
	...
	public HashMap<String, String> getAllDealershipInfo() {
		try {
			...
			if (dealershipResultSet.next()) {
				...
				dealershipInfoMap.put("available space", Integer.toString(dealershipResultSet.getInt("capacity") - (carInInventoryNum + motorInInventoryNum)));
	...
	private int getCarInInventoryNum() {
		try {
			var resultSet = DBManager.getInstance()
					.runQuery("SELECT COUNT(*) FROM vehicles WHERE inInventory = 'true' AND carType IS NOT NULL;");
			if (resultSet.next()) {
	...
	private int getMotorInInventoryNum() {
		try {
			var resultSet = DBManager.getInstance()
					.runQuery("SELECT COUNT(*) FROM vehicles WHERE inInventory = 'true' AND handleBarType IS NOT NULL;");
			if (resultSet.next()) {
				return resultSet.getInt(1);
			}
			return 0;
	...
	private int getSalesNum() {
		try {
			var resultSet = DBManager.getInstance().runQuery("SELECT SUM(price) FROM sales JOIN vehicles ON sales.vehicleID = vehicles.ID ;");
			if (resultSet.next()) {
				return resultSet.getInt(1);
			}
			return 0;
