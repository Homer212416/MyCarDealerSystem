...
public class accountManagePageController {
	...
	public void newUserSubmit(String[] newUserInfo, JFrame oldPage) {
		...
		String query = "INSERT INTO usersInfo (firstName, lastName, jobTitle, email, password, editSecurity, pageSecurity) VALUES ('"
				+ firstName
				+ "', '" + lastName + "', '" + jobTitle + "', '" + email + "', '" + password + "', '" + editSecurity
				+ "', '" + pageSecurity + "')";
		try {
			DBManager.getInstance().runInsert(query);
		} catch (SQLException e) {
